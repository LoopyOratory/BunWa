...
export class NowebPersistentStore implements INowebStore {
  ...
  private async onMessageUpdate(updates: any) {
    for (const update of updates) {
      ...
      const isYetRealMessage = isRealMessage(message) || false;
      if (isYetRealMessage) {
  ...
  private async onMessageReaction(reactions: any) {
    for (const { key, reaction } of reactions) {
      ...
      this.updateMessageWithReaction(msg, reaction);
  ...
  private async onMessageReceiptUpdate(updates: any) {
    for (const { key, receipt } of updates) {
      ...
      this.updateMessageWithReceipt(msg, receipt);
      await this.messagesRepo.upsertOne(msg);
    }
  }
  ...
  private updateMessageWithReaction(msg: any, reaction: any) {
    if (!msg.reactions) {
      msg.reactions = [];
    }
    ...
    if (existingIndex >= 0) {
      if (reaction.text === '') {
        ...
        msg.reactions[existingIndex] = reaction;
      }
  ...
  private updateMessageWithReceipt(msg: any, receipt: any) {
    if (!msg.userReceipt) {
      msg.userReceipt = [];
    }
