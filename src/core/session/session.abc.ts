import { getBrowserExecutablePath as getBrowserExecutablePathAutodetect } from './session.browser';
import {
  CoreMediaConverter,
  IMediaConverter,
} from '../media/IConverter';
import { MessagesForRead } from '../utils/convertors';
import {
  IgnoreJidConfig,
  isJidBroadcast,
  isJidGroup,
...
import { Label, LabelDTO, LabelID } from '../../structures/labels.dto';
import { LidToPhoneNumber } from '../../structures/lids.dto';
import { PaginationParams } from '../../structures/pagination.dto';
import { MessageSource, WAMessage } from '../../structures/responses.dto';
import { BrowserTraceQuery } from '../../structures/server.debug.dto';
...
export abstract class WhatsappSession {
  ...
  public constructor({
    ...
    // BehaviorSubject so that new WebSocket subscribers receive the current
    // status on connect, not just future transitions.
    this.status$ = new BehaviorSubject<WAHASessionStatus>(WAHASessionStatus.STOPPED);
