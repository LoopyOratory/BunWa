...
const QUICK_REACTIONS = [
  "\u{1F44D}", "\u{1F44E}", "\u{2764}\u{FE0F}", "\u{1F49B}", "\u{1F49A}", "\u{1F499}",
  "\u{1F49C}", "\u{1F5A4}", "\u{1F60D}", "\u{1F929}", "\u{1F970}", "\u{1F618}",
  "\u{1F602}", "\u{1F923}", "\u{1F62E}", "\u{1F631}", "\u{1F625}", "\u{1F622}",
  "\u{1F637}", "\u{1FAE0}", "\u{1F4AF}", "\u{1F525}", "\u{1F389}", "\u{1F31F}",
  "\u{1F916}", "\u{1F4AA}", "\u{1F44F}", "\u{1F64F}", "\u{1F91D}", "\u{1FAC0}",
]
...
function ChatComposer({
  ...
  const blobToBase64 = React.useCallback((blob: Blob): Promise<string> =>
    new Promise((resolve) => { const reader = new FileReader(); reader.onload = () => resolve(reader.result as string); reader.readAsDataURL(blob) }), [])

  const startVoiceRecording = React.useCallback(async () => {
    if (!onVoiceRecorded) return

    if (!navigator.mediaDevices?.getUserMedia) {
      toast.error("Voice recording requires a secure connection (HTTPS or localhost)")
      return
    }
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        onVoiceRecorded(base64, mimeType)
      }
      recorder.start()
  ...
  }, [onVoiceRecorded, blobToBase64, cleanupRecording])
  ...
  React.useEffect(() => {
    return () => { if (recordTimerRef.current) clearInterval(recordTimerRef.current); if (streamRef.current) streamRef.current.getTracks().forEach(t => t.stop()) }
  }, [])

  /* ── Popout media items ── */
  const mediaItems = [
    { label: "Image", icon: ImageIcon, type: "image" },
    { label: "File", icon: Paperclip, type: "file" },
    { label: "Voice Message", icon: Mic, type: "voice" },
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-[var(--chat-text-secondary)] leading-relaxed">
                        BunWa needs microphone access to record and send voice messages.
                      </p>
                    </div>
                  </div>
